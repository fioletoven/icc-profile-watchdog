...
public interface IByteSource
{
    ...
    /// <summary>
    /// Fill a read result with bytes to parse if it can be done without blocking.
    /// </summary>
    ...
    /// <summary>
    /// Ensures that there are unexamined bytes in the returned ReadResult. This could return previously
    /// read, but unexamined, bytes, this could read additional data from the source, or it could report
    /// that no more bytes are available.
    /// </summary>
    ...
    /// <returns>A ReadResult with the resulting data.</returns>
    ReadResult Read()
        => TryRead(out var ret) ? ret : throw new NotSupportedException("Must use an AsyncParser.");

    /// <summary>
    /// Mark the next unconsumed and unexamined byte in the source.
    /// </summary>
    /// <param name="consumed">A single position for both the next byte to examine and the next
    /// uncounsumed byte.</param>
    void AdvanceTo(SequencePosition consumed);
    
    /// <summary>
    /// Mark the next unconsumed and unexamined bytes separately.
    /// </summary>
    /// <param name="consumed">The first byte that has not been completely consumed. The next
    /// ReadAsync will begin at this byte.</param>
    /// <param name="examined">The first byte that has not been examined. The next ReadAsync will
    /// contain data beyond this byte.</param>
